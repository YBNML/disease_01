...
| P2b | YOLOv8 variants 비교 | n/s/m 3종 | **yolov8s 최고 mAP** (0.993), **yolov8n 최고 FPS** (168.9) |
| P3b | Seg 아키텍처 비교 | 4 조합 (Unet/DeepLabV3+/FPN × resnet34/efficientnet-b0) | **DeepLabV3+ 우승** (mIoU 0.978, canker IoU 0.962) |
| P4 | 장기 학습 / 양자화 / Distillation | A1, A2, G, H | yolov8m 30ep mAP **0.9945**, DeepLabV3+ 20ep mIoU **0.984**, INT8 크기 −73%, ViT→MBNet distill F1 0.962 |
...
**수확된 감귤(온주밀감)** 의 **궤양병(canker)** 을 이미지로 자동 검수. 데이터셋은 꼭지 잘린 수확 후 과일을 흰 배경에 놓고 촬영한 샘플이며, 본 프로젝트는 **수확·선별 단계의 품질 검수**를 표적으로 함. 궤양병은 감귤류 주요 질병으로 상품성·수출 검역에 직접 영향을 미치며, 자동 검수로 인력 비용·기준 일관성·저장 중 오염 확산 방지 효과를 기대.

> ⚠️ **주의**: 본 데이터셋은 과수원 나무에 달린 과일(on-tree)이 아닌 **수확 후 검수용 샘플**이므로, "in-field 조기 감별" 용도로 그대로 적용되지 않음. 현장(과수원) 적용은 별도 데이터 수집·도메인 적응이 필요.
...
### 5.5 P2b YOLOv8 variants 비교 (n/s/m, 각 5 epochs on MPS)

| Model | Params | mAP@0.5 | mAP@0.5:0.95 | Precision | Recall | Latency bs=1 (ms) | Throughput bs=16 (FPS) |
|---|---:|---:|---:|---:|---:|---:|---:|
| **yolov8n** | **3.0M** | 0.9926 | 0.9907 | 0.9648 | 0.9834 | 8.87 | **168.9** |
| **yolov8s** 🏆 | 11.1M | **0.9933** | **0.9917** | 0.9619 | 0.9806 | 17.24 | 68.3 |
| yolov8m | 25.9M | 0.9909 | 0.9860 | **0.9913** | 0.9393 | 34.91 | 30.2 |

#### 핵심 관찰
1. **모델 크기가 커져도 mAP 개선이 미미** — n(0.9926) → s(0.9933) → m(0.9909). s가 피크, m은 오히려 소폭 하락. 5 epoch로는 m의 capacity를 살리지 못함 (**under-fitting**)
2. **yolov8n이 속도 왕** — 168.9 FPS, params 3.0M. 모바일/엣지 배포 최적
3. **yolov8s가 가장 균형잡힘** — mAP 최고 + 여전히 68 FPS. **실전 권장 기본값**
4. **yolov8m은 현재 세팅에 과잉** — 파라미터는 m이 n의 8.6배인데 성능은 오히려 살짝 떨어짐. 더 긴 epoch 또는 더 어려운 데이터셋에서 효과 볼 듯
5. **Recall 격차**: m(0.939) < n(0.983) ≈ s(0.981) → m이 작은 객체 일부 놓침 (5 epoch under-training 특유 현상)

#### 배포 시나리오별 추천
| 시나리오 | 추천 모델 | 이유 |
|---|---|---|
| 실시간 모바일 검출 | yolov8n | 168 FPS, 3M params |
| 서버 정확도 우선 | yolov8s | mAP 최고, 여전히 68 FPS |
| 고정확도·저속 OK | yolov8m 필요 X | 현재 세팅에서 이득 없음. 더 긴 학습 필요 |

상세 결과: [`docs/results/2026-04-20-detection-comparison.md`](docs/results/2026-04-20-detection-comparison.md)

### 5.6 P3b Segmentation 아키텍처 비교 (4 combos, 3 epochs on MPS)

| Label | Arch | Encoder | Params | mIoU | IoU canker | Latency bs=1 (ms) | FPS |
|---|---|---|---:|---:|---:|---:|---:|
| unet_resnet34 (baseline) | Unet | resnet34 | 24.4M | 0.9616 | 0.9291 | 26.23 | 40.5 |
| **deeplabv3plus_resnet34** 🏆 | DeepLabV3+ | resnet34 | 22.4M | **0.9777** | **0.9621** | 30.89 | 36.1 |
| fpn_resnet34 | FPN | resnet34 | 23.2M | 0.9677 | 0.9400 | 24.95 | 41.6 |
| unet_efficientnet-b0 | Unet | efficientnet-b0 | **6.3M** | 0.9077 | 0.8172 | **24.63** | **43.3** |

#### 핵심 관찰
1. **DeepLabV3+ 우승** — mIoU 0.978, **궤양병 IoU 0.962** (baseline 대비 +0.033). Atrous conv + 공간 정보 복원이 병변 경계 학습에 유리
2. **Decoder 영향 > Encoder 경량화** — decoder 바꾸는 것이 encoder 교체보다 효과적 (FPN도 +0.006 개선)
3. **efficientnet-b0 encoder는 부족** — 파라미터 1/4로 줄였지만 mIoU −0.054, **궤양병 IoU −0.112** (0.82). 경량이지만 실전에 쓰기엔 성능 손실 큼
4. **Latency는 architecture 별 미미한 차이** — decoder 구조가 복잡해도 FLOPs 차이 크지 않음
5. **궤양병 클래스는 여전히 가장 어려운 클래스** — 모든 모델에서 IoU 최하. Polygon 라벨이 과일 외곽선이라는 근본적 한계 지속

#### 배포 시나리오별 추천
| 시나리오 | 추천 모델 | 이유 |
|---|---|---|
| 최고 성능 필요 | DeepLabV3+ + resnet34 | mIoU 0.978, canker IoU 0.962 |
| 속도·정확도 균형 | FPN + resnet34 | mIoU 0.968, 41 FPS |
| 극경량 배포 (엣지) | Unet + efficientnet-b0 | 6.3M params, 43 FPS — 성능 손실 감수 |

상세 결과: [`docs/results/2026-04-20-segmentation-comparison.md`](docs/results/2026-04-20-segmentation-comparison.md)

### 5.7 P4 장기 학습 / 양자화 / Distillation

#### A1 — YOLOv8m 30 epoch (under-fit 가설 검증)
P2b에서 yolov8m이 5 epoch에 mAP@0.5=0.991로 yolov8s(0.993)보다 낮아 **under-fit 가설**이 있었음. 30 epoch로 재검증:
...
**결론**: under-fit 가설 **확인**. 30 epoch에서 yolov8m이 P2b의 yolov8s 5ep(0.993)를 넘어섬. 충분히 학습하면 큰 모델이 조금 더 나은 성능 달성.

#### A2 — DeepLabV3+ 20 epoch (장기 학습)
P3b에서 3 epoch만 돌렸던 DeepLabV3+를 20 epoch로 재학습:

| Run | Epochs | mIoU | IoU bg | IoU normal | IoU canker |
|---|---:|---:|---:|---:|---:|
| P3b (3 ep) | 3 | 0.9777 | 0.998 | 0.973 | 0.962 |
| **P4 A2 (20 ep)** | 20 | **0.9836** | 0.998 | 0.981 | **0.9721** |

**결론**: 장기 학습으로 궤양병 IoU +0.010 개선. 수렴에 근접 (epoch 17~20 mIoU 거의 변화 없음).

#### G — ViT-Small INT8 양자화
...
**결론**:
- ✅ **크기 −73%** (82.7MB → 22MB): 배포 시 큰 이점
- ✅ **정확도·F1 완전 보존**: 0 pp 손실
- ⚠️ **CPU latency는 오히려 느려짐**: dynamic INT8 quant/dequant 오버헤드 > FP32 최적화 이득. 이는 Apple Silicon / 작은 ViT에서 흔한 현상
- 🎯 **메모리 제약 환경에 유용하나, 속도 목적이라면 부적합**

#### H — ViT-Small → MobileNetV3-Large 지식 증류 (Knowledge Distillation)

학생(MobileNetV3-L) + 교사(ViT-Small)로 5 epoch distillation.

| 모델 | Accuracy | F1 (canker) | AUC | Params |
|---|---:|---:|---:|---:|
| Teacher: ViT-Small | 99.06% | 0.988 | 0.999 | 21.7M |
| Student (plain, P1b) | 97.66% | 0.971 | 0.995 | 4.2M |
| **Student (distilled, P4)** | 96.96% | 0.9619 | 0.9943 | 4.2M |

**결론**:
- Distillation이 **plain training 보다 오히려 약간 낮음**
- 원인 가설: (α=0.5, T=4.0) 조합이 이 데이터/모델 쌍에 최적이 아닐 수 있음. 더 긴 학습 + α sweep 필요
- **데이터가 쉬워서 학생이 이미 충분한 성능에 도달** → distillation의 이득이 제한적
- 교훈: distillation이 항상 효과적이진 않음. baseline 모델이 이미 작지만 성능 좋으면 효과 작음

상세:
- A1: [`docs/results/2026-04-20-yolov8m-long-30ep.csv`](docs/results/2026-04-20-yolov8m-long-30ep.csv)
- A2: [`docs/results/2026-04-20-deeplabv3plus-long-20ep.json`](docs/results/2026-04-20-deeplabv3plus-long-20ep.json)
- G: [`docs/results/2026-04-20-quantization.md`](docs/results/2026-04-20-quantization.md)
- H: [`docs/results/2026-04-20-distillation.log`](docs/results/2026-04-20-distillation.log)
...
- [x] **P2b** — Detection variants 비교 (YOLOv8 n/s/m)
- [x] **P3b** — Segmentation 아키텍처 비교 (Unet/DeepLabV3+/FPN × resnet34/efficientnet-b0)
- [x] **P4 A1** — YOLOv8m 30 epoch 장기 학습 (under-fit 가설 검증)
- [x] **P4 A2** — DeepLabV3+ 20 epoch 장기 학습
- [x] **P4 G** — ViT-Small INT8 양자화
- [x] **P4 H** — ViT-Small → MobileNetV3 Knowledge Distillation
- [x] **Jupyter 분석 노트북** 8종 (06 lessons, 07 Grad-CAM, 08 FP 분석 포함)
