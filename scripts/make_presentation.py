...
def build_presentation():
    ...
    add_content_slide(prs, "프로젝트 목적",
        [
            ("문제 정의", 0, True),
            ("수확된 감귤(온주밀감)의 정상 vs 궤양병(canker) 자동 검수", 1),
            ("데이터셋은 꼭지 잘린 수확 후 과일을 배경 분리된 환경에서 촬영", 1),
            ("필요성", 0, True),
            ("궤양병은 감귤류 주요 질병 — 상품성·수출 검역에 직접 영향", 1),
            ("수확·선별 단계 자동 품질 검수로 인력 비용 절감 + 기준 일관성", 1),
            ("불량 과일 조기 제거로 유통·저장 중 오염 확산 방지", 1),
            ("접근 방법", 0, True),
    ...
    add_content_slide(prs, "데이터셋: AI Hub 감귤 병충해 영상",
        [
            ...
            "이미지 특성 — 수확·선별 단계 검수용 촬영본",
            ("꼭지 잘린 수확 후 과일을 흰 배경에 놓고 촬영", 1),
            ("1920×1080, 이미지당 감귤 1개 (단일 객체)", 1),
            "풍부한 메타데이터",
